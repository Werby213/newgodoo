...
show_guide = True
pygame.init()
pygame.display.set_caption('Newgodoo a0.1.1')
COLLTYPE_DEFAULT = 0
...
floor = pymunk.Segment(static_body, (-1000, screen_height - 100), (screen_width, screen_height - 100), 0)
floor.friction = 1.0
space.add(floor)
...
spawn_button_positions = [(button_x, button_y + (spawn_button_height + button_spacing) * i) for i in range(11)]
...
def spawn_square(position):
    ...
    moment = pymunk.moment_for_box(mass, (2*size[0], 2*size[1]))  # Calculate moment of inertia for a square
    body = pymunk.Body(mass, moment)
    body.position = position
    ...
    for body, shape in objects:
        #distance = world_mouse_pos.get_distance(body.position)
            ...
            if creating_force_field and shape.point_query(mouse_get_pos()):
                pygame.draw.circle(screen, (100, 0, 0), world_mouse_pos, force_field_radius, 2)
                force_vector = (world_mouse_pos - body.position).rotated(-body.angle).normalized() * force_field_strength * 30
...
def update():
    if running_physics == True:
        space.step(dt)
        force_field_update()
        pygame.draw.circle(screen, (255, 255, 255), world_mouse_pos, 20, 2)
...
def save_data(data):
    ...
    file_path = filedialog.asksaveasfilename(defaultextension=".grsv",
                                             filetypes=[("Newgodoo Save File", "*.grsv")])
...
def load_data():
    ...
    file_path = filedialog.askopenfilename(filetypes=[("Newgodoo Save Files", "*.grsv")])
...
while running:
    ...
    inverse_translation_cam = pymunk.Transform.translation(-translation.tx, -translation.ty)
    ...
    world_mouse_pos = (world_cursor_pos.x + world_translation.x + 300, world_cursor_pos.y + world_translation.y + 300)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.USEREVENT:
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                ...
                if event.ui_element == strength_slider:
                    ...
                    text_square_size_x.set_text("square X        :{}".format(set_square_size[0]))
                elif event.ui_element == square_size_slider_y:
                    set_square_size[1] = int(event.value)
                    text_square_size_y.set_text("square Y        :{}".format(set_square_size[1]))

                elif event.ui_element == circle_radius_slider:
                    set_circle_radius = int(event.value)
                    text_circle_radius.set_text("Circle R: {}".format(set_circle_radius))

                elif event.ui_element == iterations_slider:
                    space.iterations = int(event.value)
                    text_iterations.set_text("iterations        :{}".format(space.iterations))

                elif event.ui_element == simulation_frequency_slider:
                    simulation_frequency = int(event.value)
                    text_simulation_frequency.set_text("sim. frequency     :{}".format(simulation_frequency))
                    ...
                    text_elasticity.set_text("elasticity       :{}".format(set_elasticity))
        ...
        # Обработка событий клавиатуры
            ...
            if event.key == pygame.K_SPACE:
                ...
                if creating_static_field:
                    ...
                    static_field_start = pymunk.pygame_util.get_mouse_pos(space)
        ...
        # Обработка событий мыши
            ...
            if camera_dragging:
                current_pos = None
                translation = translation.translated(current_pos + camera_drag_start)
                camera_drag_start = current_pos
                pygame.draw.circle(screen, (100, 0, 0), current_pos, 50)
