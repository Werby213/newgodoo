...
class PhysicsDemo:
    ...
    def __init__(self):
        ...
        self.spawn_buttons = []
        self.spawn_button_width = 100
        self.spawn_button_height = 50
        self.button_x = 10
        self.button_y = 10
        self.button_spacing = 10
        self.spawn_button_positions = [(self.button_x, self.button_y + (self.spawn_button_height + self.button_spacing) * i) for i in range(4)]

        self.shape_names = ["Circle", "Square", "Triangle", "Random"]
        self.selected_shape = self.selected_shape.lower()

        self.spawn_buttons = [
            pygame_gui.elements.UIButton(
                relative_rect=pygame.Rect(pos, (self.spawn_button_width, self.spawn_button_height)),
                text=shape_name + (" (selected)" if self.selected_shape == shape_name.lower() else ""),
                manager=self.gui_manager
            )
            for pos, shape_name in zip(self.spawn_button_positions, self.shape_names)
        ]
    ...
    def spawn_item(self, position):
        if self.selected_shape == "circle":
            ...
            shape = pm.Circle(body, random.randrange(5, 30))
            shape.friction = 0.7
        ...
        elif self.selected_shape == "random":
            shape_type = random.choice(["circle", "square", "triangle"])
            self.spawn_item(position, shape_type)
    ...
    def draw_helptext(self):
        ...
        y = 5
        for line in text:
            text = font.render(line, True, pygame.Color("red"))
