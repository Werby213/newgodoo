...
textured_box_texture = pygame.image.load("laydigital.png")
...
def toolset(position):
    shape_mapping = {
        ...
        "texture test": spawn_textured_square,
    }
...
def spawn_textured_square(position):
    size = 100, 100
    points = [
        (-size[0], -size[1]),
        (-size[0], size[1]),
...
def textured_shape_update():
    # Code for displaying and rotating objects
    for body, shape in objects:
        position = body.position
        angle_degrees = -math.degrees(body.angle)

        if hasattr(shape, 'texture'):  # Check if the shape has a texture attribute
            ...
            width = max_x - min_x
            height = max_y - min_y
            texture = pygame.transform.scale(shape.texture, (int(width), int(height)))
            image = textured_box_texture
            rotated_shape = pygame.Surface((width, height), pygame.SRCALPHA)
            pygame.draw.polygon(rotated_shape, shape.color, [(v.x - min_x, v.y - min_y) for v in vertices])

            # Apply texture
            texture = pygame.transform.scale(shape.texture, (int(width), int(height)))
            rotated_shape.blit(textured_box_texture, (0, 0))
...
while running:
    ...
    for event in pygame.event.get():
            ...
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                ...
                if event.ui_element in spawn_buttons:
                    ...
                    if selected_button == spawn_buttons[0]:
                        ...
                        selected_shape = "texture test"
                    elif selected_button == spawn_buttons[7]:
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_j:
            # Начало создания пружины
            spring_start = world_mouse_pos
            creating_spring = True
        elif event.type == pygame.KEYUP and event.key == pygame.K_j:
            print("Создается пружина")
            spring_end = world_mouse_pos
            spring = pymunk.DampedSpring(anchor_a=body, anchor_b=body,
                                         a=spring_start, b=spring_end,
                                         damping=1, stiffness=1, rest_length=1)
            space.add(spring)
            creating_spring = False
